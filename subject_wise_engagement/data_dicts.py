...
def get_all_data_dicts():
    ...
    curr_plus_prev_trimesters = get_previous_trimesters(get_current_trimester())[:3] # The items of this list will act as keys of the dictionary; elements are terms in descending order, like current(t2-2025), previous(t1-2025), t3-2024 and so on
        ...
        try:
            for row in df_map_category_to_id.itertuples(): # This loop is for finding course-specific dataframes for each term
                try:
                    ...
                    # if not (category_id==26): continue # REMOVE FROM FINAL DEPLOYMENT
